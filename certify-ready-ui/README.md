...
Frontend for the Certify Ready practice app, built with React + Vite.
...
Create a .env file in this folder if your API is not served from the same origin:
...
- Framework: React 18
- Build tool: Vite
- Styling: plain CSS modules-by-convention via shared stylesheet files
- Data access: fetch-based API calls in src/api.js
- UI model: phase-driven single-page flow managed in App.jsx
...
- Component-based UI
	- reusable panels and cards are split into focused components under src/components
...
- API integration layer
	- src/api.js centralizes HTTP calls for exams, questions, validation, and result history
...
- Shared visual styles live in App.css and index.css
...
The frontend is backed by an ASP.NET Core Web API in the certify-ready-api project.
